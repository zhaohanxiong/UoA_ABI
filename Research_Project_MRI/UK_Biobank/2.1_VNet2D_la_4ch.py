...
def evaluate(test_image,test_Label,CNN_model,log_path,mu=0,sd=1):
	...
	for i in range(test_image.shape[0]):
		...
		temp      = f1_score(true,pred,average=None)[1:]
		
		if len(temp) < 2:
			temptemp = [1,1]
			temptemp[int(np.unique(true)[0]-1)] = temp[0]
			temp = temptemp
			
		# store scores
		f1_scores.append(np.mean([temp[0],temp[1]]))
	...
	f.write("\n        LA ENDO DSC = "+str(np.round(np.mean(np.array(score1)),5)))
