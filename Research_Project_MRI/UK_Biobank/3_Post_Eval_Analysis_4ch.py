...
RAendo_score = np.zeros([len(files),len(column_headers)])
...
for n in range(len(files)):
	...
	if True:
		...
		if np.sum(t) == 0 or np.sum(p) == 0:
			...
			LAendo_score[n,0:4]  = 1
			LAendo_score[n,4:12] = np.nan
			LAendo_score[n,12]   = 0
		...
		if np.sum(t) == 0 or np.sum(p) == 0:
			...
			RAendo_score[n,0:4] = 1
			RAendo_score[n,4:8] = np.nan
			RAendo_score[n,12]  = 0
			...
			# surface to surface distance: mean (average symetric surface2surface dist) max (symetric Hausdorff dist) 
			RAendo_score[n,12] = surfd(p,t,[1.8,1.8],1).mean()
