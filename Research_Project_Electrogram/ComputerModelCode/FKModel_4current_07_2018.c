...
int main(int argc, char **argv) {
	...
	while (t < 3000) {
		...
		// (D1D2=0.050/t=170/500x500/tau_fi=0.008(min), drifting rotor. tau_fi > 0.05 similar to tau_fi==0.0834,D=0.05,t=200)
		...
		if (!rank) {
		  if (snp % 400 == 0) {
			...
			sprintf (str, "Test3/aa%04d.vtk", cnt);
			out = fopen (str, "wt");
			for (z = 0; z < Z; z++) {
			  for (y = 0; y < Y; y++) {
				for (x = 0; x < X; x++)
