...
#define D1   0.06 //0.3//1.2//3.3 
#define D2   0.06 //0.06//1.2//0.33
...
int main(int argc, char **argv) {
	...
	// Loading rotor
	float temp_u[501][501][6],temp_v[501][501][6],temp_w[501][501][6],temp_s[501][501][6];
	in = fopen("StableRotor500x500/aa0503.vtk","r"); // note these are 240x240x5
	for(z = 0; z < 5; z++) {
		for(y = 0; y < 500; y++) {
			for(x = 0; x < 500; x++){
				fscanf(in,"%f ",&temp);
	...
	fclose(in);
	in = fopen("StableRotor500x500/DataV0503.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
	...
	fclose(in);
	in = fopen("StableRotor500x500/DataW0503.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
	...
	fclose(in);
	in = fopen("StableRotor500x500/DataS0503.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
				temp_s[x][y][z] = temp/100;
			}
			fscanf(in,"\n");
		}
		fscanf(in,"\n");
	...
	for (x = 0; x <= X; x++)
		for (y = 0; y <= Y; y++) 
			for (z = 0; z <= Z; z++) {
				u[x][y][z] = temp_u[x+150][y+50][z];
				v[x][y][z] = temp_v[x+150][y+50][z];
				w[x][y][z] = temp_w[x+150][y+50][z];
				s[x][y][z] = temp_s[x+150][y+50][z];
			}
	...
	*/
	...
	while (t < 500) {
		...
		// applying external stimulus or deleting areas after loading rotor does NOT generate wave break
