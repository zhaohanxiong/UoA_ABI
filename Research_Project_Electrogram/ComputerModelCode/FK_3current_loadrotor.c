...
// stable rotor tau_d = 0.125 works for 0.125 -> 0.100
// stable rotor tau_d = 0.075 works for 0.100 -> 0.050
// stable rotor tau_d = 0.025 works for 0.050 -> 0.025
...
int main(int argc, char **argv) {
	...
	// Loading rotor
	float temp_u[501][501][6],temp_v[501][501][6],temp_w[501][501][6];
	for (x = 0; x <= X; x++)
		for (y = 0; y <= Y; y++) 
			for (z = 0; z <= Z; z++) {
				temp_u[x][y][z] = 1.18*0.00000001;
				temp_v[x][y][z] = 1.0;
				temp_w[x][y][z] = 1.0; 
			}

	in = fopen("StableRotor500x500_0.125/aa0200.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
				temp_u[x][y][z] = temp/100;
			}
			fscanf(in,"\n");
		}
		fscanf(in,"\n");
	...
	fclose(in);
	in = fopen("StableRotor500x500_0.125/DataV0200.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
				temp_v[x][y][z] = temp/100;
			}
			fscanf(in,"\n");
		}
		fscanf(in,"\n");
	...
	fclose(in);
	in = fopen("StableRotor500x500_0.125/DataW0200.vtk","r");
	for(z = 0; z <= 5; z++) {
		for(y = 0; y <= 500; y++) {
			for(x = 0; x <= 500; x++){
				fscanf(in,"%f ",&temp);
				temp_w[x][y][z] = temp/100;
			}
			fscanf(in,"\n");
		}
		fscanf(in,"\n");
	...
	for (x = 0; x <= X; x++)
		for (y = 0; y <= Y; y++)
			for (z = 0; z <= Z; z++) { // note x y are flipped
				u[x][y][z] = temp_u[x+160][y+110][z];
				v[x][y][z] = temp_v[x+160][y+110][z];
				w[x][y][z] = temp_w[x+160][y+110][z];
			}

	// Computer simulation
	while (t < 10000){ // 10000 == 10s is ideal
		...
		if (!rank){
			if (snp % 400 == 0){
				...
				if (t >= 0){
					...
					out = fopen (str, "wt");
					for (z = 2; z < 3; z++) { // save the middle slice only
					  for (y = 0; y < Y; y++) {
						for (x = 0; x < X; x++)
