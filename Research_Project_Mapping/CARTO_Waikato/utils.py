...
def plot_raster_as_mesh(mask,path,PV_loc,PV_names,out_name,compute_dist=True,save=True):
	...
	# plot data
	fig = go.Figure(data=[go.Mesh3d(x=a_coords[:,0],y=a_coords[:,1],z=a_coords[:,2],
							        i=a_triang[:,0],j=a_triang[:,1],k=a_triang[:,2],color='grey',opacity=0.1)])
	...
	fig.add_trace(go.Scatter3d(x=PV_loc[0,:],y=PV_loc[1,:],z=PV_loc[2,:],
