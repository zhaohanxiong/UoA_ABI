...
# load data
dat = scipy.io.loadmat(melbourne_patients[n]+"/Mesh LA.mat")

coords     = dat['coord']
p_conchull = dat['ashape_x']

path = np.array(pd.read_csv(melbourne_patients[n]+"/Path LA.csv"))

# reshape inputs
coords[:,0] -= np.min(coords[:,0])
coords[:,1] -= np.min(coords[:,1])
coords[:,2] -= np.min(coords[:,2])
coords[:,0] = coords[:,0]/np.max(coords[:,0])*100 + 270
coords[:,1] = coords[:,1]/np.max(coords[:,1])*100 + 270
coords[:,2] = coords[:,2]/np.max(coords[:,2])*40 + 2
coords = coords.astype(np.uint32)

p_conchull[:,0] -= np.min(p_conchull[:,0])
p_conchull[:,1] -= np.min(p_conchull[:,1])
p_conchull[:,2] -= np.min(p_conchull[:,2])
p_conchull[:,0] = p_conchull[:,0]/np.max(p_conchull[:,0])*100 + 270
p_conchull[:,1] = p_conchull[:,1]/np.max(p_conchull[:,1])*100 + 270
p_conchull[:,2] = p_conchull[:,2]/np.max(p_conchull[:,2])*40 + 2
p_conchull = p_conchull.astype(np.uint32)

path[:,0] -= np.min(path[:,0])
path[:,1] -= np.min(path[:,1])
path[:,2] -= np.min(path[:,2])
path[:,0] = path[:,0]/np.max(path[:,0])*100 + 270
path[:,1] = path[:,1]/np.max(path[:,1])*100 + 270
path[:,2] = path[:,2]/np.max(path[:,2])*40 + 2
path = path.astype(np.uint32)

# Melbourne CT ------------------------------------------------------------------------------------------------

# load data
lab = np.zeros([640,640,44]).astype(np.uint8)
f2  = os.listdir(melbourne_patients[n]+"/mask")

for i in range(lab.shape[2]):
	lab[:,:,i] = cv2.imread(melbourne_patients[n]+"/mask/"+f2[i],cv2.IMREAD_GRAYSCALE)
...
for i in range(path.shape[0]):
	...
	# apply the vector from waikato surface to obtain waikato catheter path, scale the distance according to volume
...
pd.DataFrame(waikato_path).to_csv(melbourne_patients[n]+"_projected_path.csv",header=["x","y","z"],index=False)
