...
# set directories
lgemri_path = "C:/Users/zxio506/Desktop/Atria_Data/Utah/"
cloud_path  = "C:/Users/zxio506/Desktop/Atria_Data/CatheterPath/2010_BeiJing_PointCloud"
out_path    = "C:/Users/zxio506/Desktop/beijing utah path"

os.mkdir(out_path)

# define individual patients for utah and beijing
utah_patients = []
for s in os.listdir(lgemri_path):
	pat = os.listdir(lgemri_path+s)
	for s2 in pat:
		utah_patients.append(s+"/"+s2)
...
# loop through all 154 utah patients
for u in range(len(utah_patients)):

	#u = np.where(utah_patients == "CARMA0072/pre")[0][0]
	
	# define current patient name
	u_out = utah_patients[u]
	u_out = u_out.replace("/","")
	u_out = u_out.replace("CARMA","")
	
	# load utah data
	laendo          = np.rollaxis(load_nrrd(lgemri_path+utah_patients[u]+"/laendo.nrrd"),0,3)
	lawall          = np.rollaxis(load_nrrd(lgemri_path+utah_patients[u]+"/lawall.nrrd"),0,3)
	laendo_no_veins = np.rollaxis(load_nrrd(lgemri_path+utah_patients[u]+"/laendo_no_veins.nrrd"),0,3)
	
	laendo[laendo>=1] = 1
	lawall[lawall>=1] = 1
	laendo_no_veins[laendo_no_veins>=1] = 1
	...
	# loop through 10 bejing patients
	for b in range(len(beijing_patients)):
		
		print(utah_patients[u]+" - "+str(beijing_patients[b]))
		
		# load bejing data obtained via ensite software during ablation
		dat = scipy.io.loadmat(cloud_path+"/"+beijing_patients[b]+"/"+beijing_patients[b]+".mat")
		...
		for i in range(cath_path.shape[0]):
			...
			vec_x = np.linspace(utah_com[0],max_p[0],max_n)
			vec_y = np.linspace(utah_com[1],max_p[1],max_n)
			vec_z = np.linspace(utah_com[2],max_p[2],max_n)
		...
		utah_path = np.array(utah_path)
		
		### --------------------------------------------------------------------------------------------------- SAVE OUTPUT
		out_file_name = beijing_patients[b]+"_"+u_out
		#plot_raster_as_mesh(laendo,utah_path,utah_PV,utah_PV_names,out_path+"/"+out_file_name,compute_dist=False,save=True)
		pd.DataFrame(utah_path).to_csv(out_path+"/"+out_file_name+".csv",header=["x","y","z"],index=False)
