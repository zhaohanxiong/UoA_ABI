...
### Evaluation Function ------------------------------------------------------------------------------------------------------------------
def evaluate(data,CNN_model,log_path):
...
for n in range(100):
	...
	# online augmentation
	train_image,train_label = online_augmentation(data["train.data"],data["train.label"])
	
	# run 1 epoch
	model.fit(train_image,train_label,n_epoch=1,show_metric=True,batch_size=1,shuffle=True)
	#model.fit(data["train.data"],data["train.label"],n_epoch=1,show_metric=True,batch_size=1,shuffle=True)
	
	# evaluate current performance
	DSCs = evaluate(data,model,log_path)
	
	# if the model is currently the best
	if np.mean(DSCs) > best_DSC:
		best_DSC = np.mean(DSCs)
		save_best(data,model)
