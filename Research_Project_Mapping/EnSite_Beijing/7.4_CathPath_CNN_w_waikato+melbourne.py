...
### Evaluation Function ------------------------------------------------------------------------------------------------------------------
def evaluate(data,w_data,m_data,CNN_model,log_path):
	...
	# loop through melbourne patients
	for i in range(m_data["test.data"].shape[0]):
		
		print("Test "+str(i+1)+" of "+str(m_data["test.data"].shape[0]))
		
		# make prediction for slices with midpoints
		pred = np.argmax(CNN_model.predict([m_data["test.data"][i]])[0],3)
		lab  = np.argmax(m_data["test.label"][i],3)
	...
	f.write("\n  WAIKATO DSC AVEARGE = "+str(np.round(np.mean(f1_scores[-6:-2]),5))+"\n")
	f.write("\nMELBOURNE DSC AVEARGE = "+str(np.round(np.mean(f1_scores[-2:]),5))+"\n")
	f.write("\n\n")
	f.close()
	
	return(np.array(f1_scores))

def save_best(data,w_data,m_data,CNN_model):
	...
	# loop through melbourne test patients
	for i in range(m_data["test.data"].shape[0]):
		...
		# save to output
		scipy.io.savemat("Prediction Sample/melbourne_test"+"{0:03}".format(i+1)+".mat",mdict={"input":m_data["test.data"][i],"true":lab,"pred":pred})
...
melb_data    = h5py.File("melbourne path.h5","r")
...
for n in range(1000):
	...
	DSCs = evaluate(data,waikato_data,melb_data,model,log_path)
	
	# if the model is currently the best
	if np.mean([np.mean(DSCs),np.mean(DSCs[-6:-2]),np.mean(DSCs[-2:])]) > best_DSC:
		best_DSC = np.mean(DSCs)
		save_best(data,waikato_data,melb_data,model)
